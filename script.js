function submitForm() {
    ...
    window.location.href = 'result.html?caloriesNeeded=' + calorie + '&weightStatus=' + weightStatus;
}
...
function getIntensityDescription(intensity) {
    // 根據選中的工作程度返回對應的介紹文本
    if (intensity === '輕度') {
        return '大部分從事靜態或坐著的工作，例如:辦公室的上班族、銷售員';
    } else if (intensity === '中度') {
        return '從事機械操作、接待等站立較多的工作，例如:護士、服務生';
    } else if (intensity === '重度') {
        return '從事農漁業、建築等重度使用體力的工作，例如:運動員、搬家工人';
    }
}
